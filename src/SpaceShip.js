...
export function SpaceShip (props) {
    ...
    ,[gltf])
        ...
        useFrame((state,delta)=>{
            ...
            }else if (e.keyCode === 37 && props.planePosition.x === 0) { // position => medel move => left
                props.setPlanePosition(new Vector3(2,1,0))
            }else if (e.keyCode === 39 && props.planePosition.x === 0) { // position => medel move => right
                props.setPlanePosition(new Vector3(-2,1,0))
            }else if (e.keyCode === 39 && props.planePosition.x === 2) { // position => left move => right
                setPastPosition([1,0])
                props.setPlanePosition(new Vector3(0,1,0))
            }
    }
